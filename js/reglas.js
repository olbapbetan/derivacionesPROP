...
R_AndE1.post = function (dummy) {
    var newf = parse ("(" + R_AndE1.prueba.conclusion.toAscii() + ") & (" + pINP1.value + ")", parsePrincipal);
    if (newf !== null) { 
...
R_AndE2.prompt = function (n) {
    pFORM.bOK.onclick = pFORM.F (this.post, n);
    pFORM.set (msg1, [pINP1,
		document.createTextNode (tk2Char ["land"]),
		document.createTextNode (this.prueba.conclusion.toStr())]);
    pINP1.focus ();
};
R_AndE2.post = function (dummy) {
    var newf = parse ("(" + pINP1.value + ") & (" + R_AndE2.prueba.conclusion.toAscii() + ")", parsePrincipal);
    if (newf !== null) { 
	var sp1 = new Prueba (newf);
...
R_ThenI.MSGnoaplicable = "Estrategia inadecuada: observe el conector principal.";
...
R_ThenE.post = function (n) {
    var newf = parse ("(" + pINP1.value + ") -> (" + R_ThenE.prueba.conclusion.toAscii() + ")", parsePrincipal);
    if (newf !== null) { 
...
R_OrE.post = function (n) {
    var newf = parse ("(" + pINP1.value + ") \\/ (" + pINP2.value + ")", parsePrincipal);
    if (newf !== null) { 
...
R_IffE1.post = function (n) {
    var newf = parse ("(" + pINP1.value + ") <-> (" + R_IffE1.prueba.conclusion.toAscii() + ")", parsePrincipal);
    if (newf !== null) { 
	var sp1 = new Prueba (newf.hijos[0]);
	var sp2 = new Prueba (newf);
...
R_IffE2.prompt = function (n) {
    pFORM.bOK.onclick = pFORM.F (this.post, n);
    pFORM.set (msg1, [document.createTextNode (this.prueba.conclusion.toStr()), 
		document.createTextNode (tk2Char ["liff"]),
		pINP1]);
    pINP1.focus ();
};
R_IffE2.post = function (n) {
    var newf = parse ("(" + R_IffE2.prueba.conclusion.toAscii() + ") <-> (" + pINP1.value + ")", parsePrincipal);
    if (newf !== null) { 
...
R_NegE.prompt = function (n) {
    pFORM.bOK.onclick = pFORM.F (this.post, n);
    pFORM.set (msg1, [document.createTextNode (tk2Char ["lnot"]),
		pINP1]);
    pINP1.focus ();
};
...
R_RAA.post = function (n) {
    var hip = parse ("- (" + R_RAA.prueba.conclusion.toAscii() + ")", parsePrincipal);
    var sp1 = new Prueba (parse ("False", parsePrincipal)); 
    sp1.hips = [hip];
