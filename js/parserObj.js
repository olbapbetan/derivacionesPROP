...
{	
    ...
    popUp.document.write("<div style='font-family: courier new; font-size:small;'>"+latexCode+"</div>");
    popUp.focus();
}
...
function switchSelectOn  () { switchSelect (false); }
function switchSelectOff () { switchSelect ( true); }
...
{
    var myIndex = precedencia(f.tk), ans = "";
    switch (lst.length)
    {
    case 0: 
	switch (f.tk)
	...
	{
	case "lall":
	case "lexi": con = "(" + con + " " + f.value + ")"; break;
	case "lnot": break;
	}
	...
	if(myIndex > rIndex) { ans = con + " (" + lst[1] + ")";}
	else { ans = con +  " " + lst[1];}
	if(myIndex >= lIndex) { ans = "(" + lst[0] + ")" + ans;}
	else { ans = lst[0] + ans;}
	break;
    default:  alert ("ERROR: cantidad de subfórmulas no válida"); break;
    }
  return ans;
}
...
function appHip (hip, n)
{
	...
	if (problema) 
	    alert ("Esta regla no está siendo aplicada correctamente.");
	else{
	    p.just = "Hip";
	    prueba2Html ();
	}
...
function menuDerivar()
{
	...
	document.getElementById ("inpTipoSimilaridad").
	appendChildren (dTS.dSim.inp, dTS.dLen.inp, dTS.dOK);
    document.getElementById ("inpPremisas").
	appendChildren (dPrems.inp, dPrems.but);
    document.getElementById ("inpConclusion").
	appendChildren (dConc.inp, dConc.but);
    document.getElementById ("botonera").
	appendChildren (dBoton.derivar, 
			dBoton.reset, 
			dBoton.help, 
			dBoton.toTex);
    reset();
...
function setTipoLenguaje (tipo, leng)
{
	...
	problemas = (arisP.length !== nmP.length) 
	...
	dPrems.but.disabled = false; dConc.but.disabled = false;
    }
    else {
	alert ("Problemas con el tipo y/o el lenguaje");
	dPrems.but.disabled = true; dConc.but.disabled = true;
    }
}
...
{
    premisas =  [];
    var ans = "", premisasTxt = fmltxt.split(";"), newf, idx = 0;
...
function parse (str, parseFunction) 
{
	...
	for( var i = 0; i < error_count; i++ ) 
	    ...
	    ( str.substr( 0, error_offsets[i] ).match( /\n/g ) ? 
	      str.substr( 0, error_offsets[i] ).match( /\n/g ).length : 1 ) 
	    + " near \"" + str.substr( error_offsets[i] ) + "\", expecting \"" 
