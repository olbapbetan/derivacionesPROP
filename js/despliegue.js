...
function puntosTrivial (ans, fSize) {
    ans.A = lenPad; ans.B = lenPad + fSize;
    ans.C = ans.A; ans.D = ans.A; ans.E = ans.B;
    ans.X = 0; ans.Z = ans.B + lenPad;
}
...
function puntosIncompleta (ans, fSize) {
    ans.A = lenPad; ans.B = lenPad + fSize;
    ans.C = ans.A; ans.D = ans.B; ans.E = ans.B + widthSelect;
    ans.X = 0; ans.Z = ans.E;
}
function cajaIncompleta (f) {
    var t = document.createCosa("TABLE");
    var tr = document.createElement("TR");
    var td1 = document.createCosa(
	"TD", 
	"style", padLeft);
...
function puntosComplejaConcMuyCorta (ans, fSize, lst, acum2, just) {
    ans.X = 0; 
    ans.C = lst[0].A; ans.D = ans.C + acum2;
...
function puntosComplejaConcMuyLarga (ans, fSize, lst, acum2, just) {
    ans.X = 0;
    ans.A = 0; ans.B = fSize;
    ans.C = ans.A; ans.D = ans.B; ans.E = ans.D + justSize[just]; 
...
function puntosComplejaConclusion (ans, fSize, lst, acum2, just) {
    ans.X = 0;
    ans.A = lst[0].A - centrado (acum2, fSize); ans.B = ans.A + fSize;
    ans.C = ans.A; ans.D = ans.B; ans.E = ans.D + justSize[just]; 
    for (i = 0; i+1 < lst.length; i ++) {
	lst[i+1].X = lst[i].Z; 
	lst[i+1].Z = lst[i+1].X + lst[i+1].Z;
...
function cajaCompleja (f, pt) 
{
    ...
    var td2 = document.createCosa (
    	"TD", 
    	"style", "padding-left:" + pt.A + "px", 
    	"ondblclick", "deshacerPrueba("+f.idx+")");
    var td3 = document.createCosa (
	"TD", 
    	"class", "just",
	"style", "width:" + justSize [f.just] + "px",
    	"ondblclick", "colapsarPrueba("+ f.idx +")");
...
function puntosInvisible (ans){
    ans.A = 0; ans.B = widthCollapsed;
    ans.C = 0; ans.D = 0; ans.E = ans.B;
    ans.X = ans.A; ans.Z = ans.B;
};
...
function dispPruebaHTML (f, lst) {
	...
	if (acum2 >= fSize) 
	    ...
	    puntosComplejaConclusion (ans, fSize, lst, acum2, f.just);
	
	var upper = document.createCosa (
	    "div", "class", "upper", 
	    "style", "padding-left:" + lst[0].X + "px");
